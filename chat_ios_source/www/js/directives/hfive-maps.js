App.directive('hfiveMaps', function() {
    return {
        restrict: 'A',
        scope: {
            map: "=",
        ...
        controller: function($scope, HfiveGoogleMaps) {

            console.log("$scope.config: ", $scope.config);

            $scope.map = HfiveGoogleMaps.createMap("google-maps");
            ...
            if($scope.config.coordinates) {
                ...
                HfiveGoogleMaps.calculateRoute($scope.config.coordinates.origin, $scope.config.coordinates.destination).then(function(route) {
                    HfiveGoogleMaps.addRoute(route);
                });
            ...
            if($scope.config.markers && $scope.config.markers.constructor === Array) {
                ...
                for(var i = 0; i < $scope.config.markers.length; i++) {
                    ...
                    HfiveGoogleMaps.addMarker(marker);
                }

                if($scope.config.bounds_to_marker) {
                    var nearbymarker=new Array();

                    for(var i = 0; i < $scope.config.markers.length; i++) {
                        if($scope.config.markers[i].distance<20000000000)
                            nearbymarker.push($scope.config.markers[i]);
                    }
                   
                    if(nearbymarker.length==0){
                        nearbymarker=($scope.config.markers);
                    }

                    console.log("newrby",nearbymarker);

                    var bounds = HfiveGoogleMaps.getBoundsFromPoints(nearbymarker);
                    HfiveGoogleMaps.fitToBounds(bounds);
                }
