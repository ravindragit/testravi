...
}).controller('TopicController', function($cordovaDevice, $rootScope, $scope, $stateParams, $timeout, $translate, $window, Application, Topic) {

    $scope.value_id = Topic.value_id = $stateParams.value_id;

    try {
        if(!Application.is_webview) {
            $scope.device_uid = Topic.device_uid = $cordovaDevice.getUUID();
        }
    } catch(e) {
